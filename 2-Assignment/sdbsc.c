...
int get_student(int fd, int id, student_t *s){
    //Calculate offset and move to that position on file
    int offset = id * sizeof(student_t);
    if (lseek(fd, offset, SEEK_SET) == -1) {
        return ERR_DB_FILE;
    }

    //Read record and check if it is empty
    student_t temp;
    student_t empty = {0};
    if (read(fd, &temp, sizeof(student_t)) <= 0 || memcmp(&temp, &empty, sizeof(student_t)) == 0) {
        return SRCH_NOT_FOUND;
    }

    //Set s to the record that was found
    *s = temp;
    return NO_ERROR;
}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    //Create new student with provided information
    student_t new;
    new.id = id;
    ...
    //Calculate index and move to that position in file
    int index = id * sizeof(student_t);
    ...
    //Write new record
    if(write(fd, &new, sizeof(student_t)) != sizeof(student_t)){
...
int del_student(int fd, int id){
    ...
    //Calculate offset and move to that position in file
    int offset = id * sizeof(student_t);
    ...
    //Rewrite the found student record with all zeros
    student_t empty = {0};
...
int count_db_records(int fd){
    ...
    size_t bytesRead = read(fd, &student, offset);

    if(bytesRead < 0) {
        printf(M_ERR_DB_READ); //Unable to read
        return ERR_DB_FILE;
    }
    else if(bytesRead == 0) {
        ...
        //Loop through contents of file to get number of records
        while(read(fd, &student, offset) > 0) {
            if(memcmp(&student, &empty, offset) != 0) {
                count++;
            }
        }
    }
...
int print_db(int fd){
    ...
    student_t student;
    student_t empty = {0};
    size_t offset = sizeof(student_t);

    //Check if database is empty
    if(read(fd, &student, offset) == 0) {
        printf(M_DB_EMPTY);
        return NO_ERROR;
    }
    ...
    //Print contents of database file as a table
    printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA");
    while(read(fd, &student, offset) > 0)
    {
        if(memcmp(&student, &empty, sizeof(student_t)) != 0) {
            float gpa = student.gpa / 100.00;
...
void print_student(student_t *s){
    //Check if provided pointer is valid
    if(s == NULL || s->id == 0) {
...
int compress_db(int fd){
    ...
    //Attempt to create temp file, raise error on failure
    int temp = open_db(TMP_DB_FILE, false);
    if(temp == -1) {
        printf(M_ERR_DB_OPEN);
        return ERR_DB_FILE;
    }

    //Loop through records of databse file and write valid records to temp file
    student_t student;
    student_t empty = {0};
    while(read(fd, &student, sizeof(student_t)) > 0) {
        if(memcmp(&empty, &student, sizeof(student_t)) == 0) {
            continue;
        }
        if(lseek(temp, student.id * sizeof(student_t), SEEK_SET) < 0) {
            printf(M_ERR_DB_READ);
            return ERR_DB_FILE;
        }
        if(write(temp, &student, sizeof(student_t)) != sizeof(student_t)) {
            printf(M_ERR_DB_WRITE);
            return ERR_DB_FILE;
        }
    }

    //Close both files for changes to take effect
    close(fd);
    close(temp);

    //Attempt to rename temp file, raise error on failure
    if (rename(TMP_DB_FILE, DB_FILE) != 0) {
        printf(M_ERR_DB_OPEN);
        return ERR_DB_FILE;
    }

    //Attempt to open renamed temp file, raise error on failure
    int new = open_db(DB_FILE, false);
    if (new == -1) {
